...
```{r rdu-flights-summ}
rdu_flights %>%
  summarise(mean_dd = mean(dep_delay), sd_dd = sd(dep_delay), iqr_dd = IQR(dep_delay), n = n())
```
...
sfo_feb_flights %>%
  group_by(carrier) %>%
  summarise(median_arr_dd= median(arr_delay), iqr_arr_dd = IQR(arr_delay))
```
...
nycflights %>% filter(dep_type == "delayed") %>% summarise(ot_arr_rate = sum(arr_type == "on-time") / n())
```